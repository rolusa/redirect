...
install_nodejs() {
    ...
    if ! command -v npm &> /dev/null; then
        log_info "安装npm包管理器..."
        if $SUDO apt-get install -y npm > /tmp/npm_install.log 2>&1; then
            log_info "  ✓ npm软件包安装完成"
        else
            log_warn "npm从apt安装失败，尝试使用n模块安装..."
            # 如果apt安装失败，使用Node.js的n模块
            if command -v node &> /dev/null; then
                $SUDO npm install -g npm@latest > /dev/null 2>&1 || true
            fi
        fi
    fi
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null; then
        ...
        log_error "尝试手动安装npm: apt-get install -y npm"
        exit 1
    fi
}
...
install_ufw() {
    ...
    log_warn "检测到云服务器环境"
    log_warn "UFW防火墙可能与云服务商安全组冲突"
    log_warn "建议优先使用云服务商的安全组管理端口"
    
    log_info "配置防火墙规则（但不启用）..."
    
    # 配置规则但不启用
    $SUDO ufw --force allow 22/tcp > /dev/null 2>&1
    log_info "  ✓ 已配置SSH端口 (22)"
    
    $SUDO ufw --force allow 80/tcp > /dev/null 2>&1
    $SUDO ufw --force allow 443/tcp > /dev/null 2>&1
    log_info "  ✓ 已配置HTTP/HTTPS端口 (80, 443)"
    
    $SUDO ufw --force allow 3000/tcp > /dev/null 2>&1
    $SUDO ufw --force allow 3001/tcp > /dev/null 2>&1
    log_info "  ✓ 已配置应用端口 (3000, 3001)"
    
    # 配置默认策略
    $SUDO ufw default deny incoming > /dev/null 2>&1
    $SUDO ufw default allow outgoing > /dev/null 2>&1
    
    log_info "✓ 防火墙规则已配置（未启用）"
    log_warn "⚠️  UFW未启用，请使用云服务商安全组管理防火墙"
    log_info "  如需手动启用UFW: sudo ufw enable"
    log_info "  配置端口: 22(SSH), 80(HTTP), 443(HTTPS), 3000(主应用), 3001(管理后台)"
}
